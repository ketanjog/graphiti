...
async function resizeResolver(source, args, actions) {
  ...
  const src = `${(0, _urlGenerator.generatePublicUrl)(source)}/${(0, _urlGenerator.generateImageArgs)({ ...args,
    width,
    height,
    format
  })}/${_path.default.basename(source.filename, _path.default.extname(source.filename))}.${format}`;
  return {
    src,
    width,
    height
...
function generateResizeFieldConfig(enums, actions) {
  return {
