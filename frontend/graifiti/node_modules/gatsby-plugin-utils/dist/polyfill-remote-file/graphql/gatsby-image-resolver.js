...
async function gatsbyImageResolver(source, args, actions) {
    ...
    const images = imageSizes.sizes.map(width => {
      if ((0, _dispatchers.shouldDispatch)()) {
        (0, _dispatchers.dispatchLocalImageServiceJob)({
          url: source.url,
          extension: format,
          basename: _path.default.basename(source.filename, _path.default.extname(source.filename)),
          width,
      ...
      const src = `${(0, _urlGenerator.generatePublicUrl)(source)}/${(0, _urlGenerator.generateImageArgs)({
        width,
        height: Math.round(width / imageSizes.aspectRatio),
        format,
        cropFocus: args.cropFocus,
        quality: args.quality
      })}/${encodeURIComponent(_path.default.basename(source.filename, _path.default.extname(source.filename)))}.${format}`;
  ...
  if (args.placeholder !== `none`) {
    ...
    } = await (0, _placeholderHandler.generatePlaceholder)(source, args.placeholder);
...
function generateGatsbyImageFieldConfig(enums, actions) {
  return {
    type: `JSON`,
